...
class NoteSummarizer {
    static let session = try LanguageModelSession(instructions: """
        You are an AI study assistant. 
        Your job is to take a student's raw lecture notes or textbook excerpts 
        and summarize them into 3-5 concise, clear bullet points that are easy to review for exams.
        Do not include unnecessary commentary, just focus on making study notes. ONLY INCLUDE THE SUMMARIZED NOTES. DO 
        NOT INCLUDE AN INTRODUCTION, CONCLUSION.
        """)
    
    static func summarizeNotes(_ text: String) async throws -> String {
        let userPrompt = """
           Summarize the following notes into 3-5 bullet points:\n\(text)
           """
